...
def kitti_active_tfrecords(
    ...
    for img_name in images:
        ...
        image_path = os.path.join(image_dir, img_name)
        annotation_for_image = filter_annotations(img_anno, classes_to_use)
        if "depth" in output_path:
            depth_path = os.path.join(output_path, img_name.split(".")[0]+"_depth.npy")
        else: depth_path = None
        example = prepare_example(image_path, annotation_for_image, label_map_dict, depth_path)
...
def prepare_example(image_path, annotations, label_map_dict, dept_path=None):
    ...
    if dept_path is not None:
        with tf.io.gfile.GFile(dept_path, "rb") as fid:
            depth_array = np.load(fid)
    ...
    if dept_path is not None:
        freatures_dict["image/encoded_depth"] = dataset_util.bytes_feature(encoded_depth_bytes)
        freatures_dict["image/depth_shape"] = dataset_util.int64_list_feature(depth_array.shape)
...
def read_annotation_file(filename):
    ...
    if "pseudoscore" in filename or "imscore" in filename or "depth" in filename:
