# adapted from https://github.com/ultralytics/ultralytics/blob/main/ultralytics/cfg/datasets/VisDrone.yaml
...
DATASET_ROOT = Path("/path/to/data/DepthPrior/datasets/visdrone")  # dataset root dir
    ...
    for f in pbar:
        ...
        with open(f, encoding="utf-8") as file:  # read annotation.txt
            for row in [x.split(",") for x in file.read().strip().splitlines()]:
                ...
                with open(
                    str(f).replace(f"{os.sep}annotations{os.sep}", f"{os.sep}labels{os.sep}"),
                    "w",
                    encoding="utf-8",
                ) as fl:
