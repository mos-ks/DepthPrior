...
class DetectionInputProcessor(InputProcessor):
    ...
    def random_horizontal_flip(self):
        ...
        self._image, self._boxes, flipped_bool = preprocessor.random_horizontal_flip(
...
class InputReader:
    ...
    def __init__(
        ...
        self.activate_depth_array = activate_depth_array
    ...
    def dataset_parser(self, value, example_decoder, anchor_labeler, params):
        ...
        with tf.name_scope("parser"):
            ...
            if self.activate_depth_array:
                depth_array = data["depth_image"]
                # depth_shape = data["depth_image_shape"]
            ...
            if self._is_training:
                ...
                if params.get("grid_mask", None):
                    from aug import \
                        gridmask  # pylint: disable=g-import-not-at-top

                    image, boxes = gridmask.gridmask(image, boxes)

                if params.get("autoaugment_policy", None):
                    from aug import \
                        autoaugment  # pylint: disable=g-import-not-at-top
            ...
            if self._is_training:
                if params["input_rand_hflip"]:
                    flipped_bool = input_processor.random_horizontal_flip()
            ...
            if self.activate_depth_array:
                if self._is_training and params["input_rand_hflip"]:
                    ...
                    depth_array = tf.cond(
                        flipped_bool,
                        lambda: _flip_image(depth_array),
                        lambda: depth_array,
                    )
                input_processor._image = depth_array
                depth_array = input_processor.resize_and_crop_image()
                extracted_data += (depth_array,)
    ...
    def process_example(
        ...
        if pseudo_score is not None and len(pseudo_score.shape) > 3:
            depth_array = pseudo_score
            pseudo_score = None
        if pseudo_score is not None:
            gt.append(pseudo_score)
        groundtruth_data = tf.concat(gt, axis=2)
        if depth_array is not None:
            labels["depth_array"] = depth_array
    ...
    def __call__(self, params, input_context=None, batch_size=None, names=False):
        ...
        example_decoder = tf_example_decoder.TfExampleDecoder(
            ...
            activate_depth_array=self.activate_depth_array,
        )
