...
class BoxNet(tf.keras.layers.Layer):
        ...
        **kwargs
        ...
        for i in range(self.repeats):
            ...
            self.boxes = self.boxes_layer(
                separable_conv, num_anchors, data_format, name="box-predict", predict_distance=predict_distance
            )
    ...
    @classmethod
    def boxes_layer(cls, separable_conv, num_anchors, data_format, name, predict_distance=False):
        """Gets the conv2d layer in BoxNet class."""
        
        # Only add distance output during training
        if predict_distance:
            extend_output = 4*num_anchors+1
        else: 
            extend_output = 4*num_anchors
        
        if separable_conv:
            return tf.keras.layers.SeparableConv2D(
                filters=extend_output,
                depth_multiplier=1,
                pointwise_initializer=tf.initializers.variance_scaling(),
                depthwise_initializer=tf.initializers.variance_scaling(),
                data_format=data_format,
