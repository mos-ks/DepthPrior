...
def define_flags():
    ...
    flags.DEFINE_string(
        "depth_images_path",
        default=None,
        help="Path to depth npys.",
    )
    flags.DEFINE_float(
        "depth_additional_parameter",
        default=0.0,
    ...
    model.compile(
        ...
        loss={
            train_lib.BoxLoss.__name__: train_lib.BoxLoss(
                ...
                loss_depth=config.predict_distance,
            ...
            tf.keras.losses.MeanAbsoluteError.__name__: tf.keras.losses.MeanAbsoluteError(
                reduction=tf.keras.losses.Reduction.NONE
            ),
        },
    )
...
def main(_):
    ...
    if "predict_distance" in FLAGS.model_dir: config.predict_distance = True  # its already false in hparams_config
    logging.info(f"predict_distance is set to: {config.predict_distance}")
    ...
    def get_dataset(
        ...
        return dataloader.InputReader(
            ...
            activate_depth_array=activate_depth_array,
    ...
    with ds_strategy.scope():
        ...
        activate_depth_array = False
        activate_aug = False
        if "depth" in FLAGS.train_file_pattern:
            activate_depth_array = True
            logging.info("Depth activated")
        ...
        if "curriculum_learning" in FLAGS.train_file_pattern:
            ...
            normal_tf_pseudo = normal_tf_pseudo[0] + "_common.tfrecord"
            train_dataset_norm = get_dataset(
                normal_tf_pseudo,
                True,
                config,
                activate_pseudo_score=activate_im_score or activate_depth_array,
                activate_depth_array=activate_depth_array,
                batch_size=FLAGS.batch_size - 1,
            )

            hard_tf_pseudo = FLAGS.train_file_pattern.split(".tfrecord")
            hard_tf_pseudo = hard_tf_pseudo[0] + "_rare.tfrecord"
        ...
        print(config)
        model.config.depth_additional_parameter = FLAGS.depth_additional_parameter
        model.config.depth_batch = True if "depth_batch" in FLAGS.model_dir else False
        model.config.depth_image = True if "depth_image" in FLAGS.model_dir else False
        model.config.depth_detection = (
            True if "depth_detection" in FLAGS.model_dir else False
        )
        model.config.rampup_det_losses = (
            True if "rampup" in FLAGS.model_dir else False
        )
        model.config.depth_loss = True if "depth_loss" in FLAGS.model_dir else False
        model.config.activate_im_score = activate_im_score


        # Convert list to a string and remove brackets if present
        loss_weight_str = " ".join(FLAGS.two_losses_weight).strip("[]").strip()

        # Ensure proper list format by replacing spaces with commas if necessary
        if ' ' in loss_weight_str and ',' not in loss_weight_str:
            loss_weight_str = loss_weight_str.replace(' ', ',')

        # Now parse the corrected string as a list safely
        model.config.two_losses_weight = ast.literal_eval(f'[{loss_weight_str}]')
        ...
        for epoch in range(initial_epoch, FLAGS.num_epochs):
            ...
            for step, batch in enumerate(train_dataset):
                ...
                # image_np = x_batch_train[0].numpy()  # RGB image, shape: (height, width, 3)
                # depth_np = y_batch_train["depth_array"][0, :, :, 0].numpy()  # Depth map, shape: (height, width)

                # # Normalize the depth map for visualization
                # depth_normalized = (depth_np - depth_np.min()) / (depth_np.max() - depth_np.min())

                # depth_weight = 0.6
                # image_weight = 1 - depth_weight  # 0.4
                # import numpy as np
                # # Convert depth to 3 channels for blending
                # depth_colored = np.stack([depth_normalized] * 3, axis=-1)  # Shape: (height, width, 3)

                # # Create an overlay by blending the RGB image and the depth map
                # overlay = image_np * image_weight + depth_colored * depth_weight

                # # Clip values to ensure they are in a valid range (0 to 1 if normalized, or 0-255 if in uint8 format)
                ...
                # axs[0].axis("off")
                # axs[1].imshow(depth_normalized, cmap="gray")
